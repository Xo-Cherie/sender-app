...
function buildSignupUrl(payload: InvitePayload) {
  const url = new URL('/login', appUrl);
  url.searchParams.set('screen', 'signUp');
  ...
  if (payload.cardId) {
    url.searchParams.set('cardId', payload.cardId);
  }

  return url.toString();
}

function inviteMessage(payload: InvitePayload) {
  const inviteUrl = buildSignupUrl(payload);
  const sender = payload.inviterName?.trim() || 'Someone';
  return `${sender} invited you to join Xo Cherie so you can send and save digital greeting cards together: ${inviteUrl}`;
}
...
function cardInviteMessage(payload: InvitePayload) {
  ...
  return `${sender} sent you ${title} on Xo Cherie. Sign up to open it: ${inviteUrl}`;
}

function inviteHtml(payload: InvitePayload) {
  const inviteUrl = escapeHtml(buildSignupUrl(payload));
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #1A1A1A; line-height: 1.5;">
      ...
      <p>
        <a href="${inviteUrl}" style="display: inline-block; background: #C17B66; color: #FFFFFF; padding: 12px 18px; border-radius: 10px; text-decoration: none; font-weight: 700;">
          Join Xo Cherie
        </a>
      </p>
...
function cardInviteHtml(payload: InvitePayload) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #1A1A1A; line-height: 1.5;">
      ...
      <p>Use the same email address this invite was sent to. Your card will appear in your inbox after you sign up.</p>
      <p>If the button does not work, open this link:</p>
      <p><a href="${inviteUrl}">${inviteUrl}</a></p>
    </div>
  `;
...
async function sendEmailInvite(email: string, payload: InvitePayload) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      text: isCardInvite ? cardInviteMessage(payload) : inviteMessage(payload),
      html: isCardInvite ? cardInviteHtml(payload) : inviteHtml(payload),
    }),
  });
...
async function sendSmsInvite(phone: string, payload: InvitePayload) {
  ...
  const body = new URLSearchParams({
    ...
    Body: payload.type === 'card' ? cardInviteMessage(payload) : inviteMessage(payload),
  });
